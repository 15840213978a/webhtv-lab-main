...
public class PlayerOsdController {
    ...
    private boolean showTitleOverlayWhenControls = true;
    ...
    // 竖屏直播内嵌态 vs 全屏态顶部右侧按钮组宽度不同：内嵌态右侧按钮更少，时间可更贴近屏右；
    // 全屏态右侧按钮更多（如 lock 出现），时间需留出更大边距避免压住按钮。两者共用同一份 OSD 布局，
    // 故按 fullscreen 状态切换 osdTopRight 的 marginEnd（默认按全屏处理，VOD 等同效）。
    private static final int TOP_RIGHT_MARGIN_EMBEDDED_DP = 46;
    private static final int TOP_RIGHT_MARGIN_FULLSCREEN_DP = 76;
    private boolean fullscreen = true;
    private int topRightMarginEndPx = -1;
    ...
    /**
     * 控制栏自身已显示标题/分辨率（如移动端直播/点播的 control.title/size）时，
     * 设为 false 可禁止 OSD 在控制栏显示时再叠加左上角名字+分辨率，避免与返回按钮重叠。
     * 默认 true（沿用 leanback 行为：控制栏无 title/size，由 OSD 提供）。
     */
    public void setShowTitleOverlayWhenControls(boolean show) {
        if (this.showTitleOverlayWhenControls == show) return;
        this.showTitleOverlayWhenControls = show;
        if (started) render();
    }

    /**
     * 竖屏直播全屏/内嵌切换时调用：全屏态顶部右侧按钮更多，时间需更大 marginEnd；
     * 内嵌态右侧按钮更少，时间可更贴近屏右。render() 每帧复用此值，旋转后也能保持正确。
     */
    ...
    private void applyTopRightMargin() {
        if (topRight == null) return;
        ViewGroup.LayoutParams lp = topRight.getLayoutParams();
        if (!(lp instanceof ViewGroup.MarginLayoutParams)) return;
        int dp = fullscreen ? TOP_RIGHT_MARGIN_FULLSCREEN_DP : TOP_RIGHT_MARGIN_EMBEDDED_DP;
        int px = Math.round(dp * App.get().getResources().getDisplayMetrics().density);
        topRightMarginEndPx = px;
        ViewGroup.MarginLayoutParams mlp = (ViewGroup.MarginLayoutParams) lp;
        if (mlp.getMarginEnd() == px) return;
        mlp.setMarginEnd(px);
        topRight.setLayoutParams(mlp);
    }
    ...
    private boolean render() {
        ...
        if (controlsVisible) {
            stopDiagnosticsSampling();
            // 控制栏自身已显示标题/分辨率时，OSD 左上角的名字+分辨率属于重复绘制，
            // 且在竖屏直播会压在返回按钮上，因此由调用方决定是否在控制栏显示时叠加左上角标题。
            if (showTitleOverlayWhenControls) setTopLeftForControls(player);
            else topLeft.setVisibility(View.GONE);

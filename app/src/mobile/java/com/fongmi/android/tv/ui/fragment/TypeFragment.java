...
import com.fongmi.android.tv.content.ActionCardHelper;
...
public class TypeFragment extends BaseFragment implements CustomScroller.Callback, VodAdapter.OnClickListener, SwipeRefreshLayout.OnRefreshListener {
    ...
    /** 实验室动作卡片内置指令：刷新 / 源内搜索 / 跳转播放。 */
    @Subscribe(threadMode = ThreadMode.MAIN)
    public void onActionCardEvent(ActionCardEvent e) {
        if (e.kind == ActionCardEvent.Kind.REFRESH) {
            // 刷新不导航，允许多个 fragment 各自刷新（保证当前可见页被刷新到）
            onRefresh();
            return;
        }
        // SEARCH / PLAY 会拉起新 Activity，必须只由第一个收到事件的 fragment 执行一次，
        // 否则 ViewPager 中多个 fragment 会各拉起一个 Activity（返回要按多次）。
        if (e.consumed) return;
        e.consumed = true;
        if (e.kind == ActionCardEvent.Kind.SEARCH && e.keyword != null) {
            // 对齐主页右上角搜索按钮：直接拉起 APP 内全局搜索页
            SearchActivity.start(requireActivity(), e.keyword);
        } else if (e.kind == ActionCardEvent.Kind.PLAY && e.url != null) {
            // 推送播放（与 HomeActivity 收到 PUSH 事件一致）
            VideoActivity.push(requireActivity(), e.url);
        }
    }
    ...
    public void onItemClick(Vod item) {
        if (item.isAction()) {
            ...
            ActionCardHelper.handleAction(requireActivity(), getKey(), item.getAction());
